...
public abstract class Filter<T> {
	...
	public enum ModKind implements ConvertibleEnum<Byte, ModKind> {
		COUNT(1) {
			@Override
			public Filter<?> createFilter(ByteBuffer bytes) throws JdwpError {
		...
		public abstract Filter<?> createFilter(ByteBuffer bytes) throws JdwpError;
	}
	...
	public static Filter<?> factory(ByteBuffer bytes) throws JdwpError {
		return ModKind.COUNT.convert(bytes.get()).createFilter(bytes);
	}
