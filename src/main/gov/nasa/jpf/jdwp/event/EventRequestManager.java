...
public class EventRequestManager {
   ...
   * The VM Start Event and VM Death Event are automatically generated events. <br/>
  ...
  public void registerSyntheticEventRequests() {
     ...
     */
    requestEvent(EventRequest.nullRequestIdFactory(EventKind.VM_INIT, Jdwp.suspendOnStartup() ? SuspendPolicy.ALL : SuspendPolicy.NONE,
                                                   null));
  ...
  public SuspendPolicy
      populateMatchedEventsAndCalculateSuspension(Event event, List<Event> matchedEvents, SuspendPolicy resultSuspendPolicy) {
  ...
  private class EventRequestTable<T extends Event> {
     ...
     * multiple matching requests) are found. These matching requests are paired
     * with the given event and inserted into the list of matching event
     * requests.
     * 
     ...
     * @param suspendPolicy
     *          The incoming suspend policy that might be updated in a return
     *          value.
     * @return Updated suspend policy if it is more restrictive than the
     *         incoming one
     */
    public synchronized SuspendPolicy populateMatchedEventsAndCalculateSuspension(T event, List<Event> matchedEvents,
